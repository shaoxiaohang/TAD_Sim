 ...
 * @param _id : sensor id
 */
CameraSensor::CameraSensor(int _id) : FovFilterSensor(_id) {
...
/// @brief all cameras
std::map<int, std::shared_ptr<CameraSensor>> cameras;
std::map<int, std::shared_ptr<CameraSensor>> semantics;
...
bool LoadCamera(const sim_msg::Sensor &sensor, const std::string &device) {
  ...
  auto cam = std::make_shared<CameraSensor>(sensor.extrinsic().id());
  // Load camera extrinsic parameters
  cam->setPosition(Eigen::Vector3d(sensor.extrinsic().locationx() * 0.01, sensor.extrinsic().locationy() * 0.01,
                                   sensor.extrinsic().locationz() * 0.01));
  cam->setRotation(a2r * sensor.extrinsic().rotationx(), a2r * sensor.extrinsic().rotationy(),
  ...
  GetPropValue(sensor.intrinsic().params(), "IntrinsicType", sbuf);
  GetPropValue(sensor.intrinsic().params(), "IntrinsicParamType", sbuf);

  if (sbuf == "0") {
    // if has introinsic matrix
    std::string instring;
    ...
    GetPropValue(sensor.intrinsic().params(), "Distortion", sbuf);
    GetPropValue(sensor.intrinsic().params(), "Distortion_Parameters", sbuf);

    // read distortion parameters
    if (!sbuf.empty()) {
      std::replace(sbuf.begin(), sbuf.end(), ',', ' ');
      double dis[5] = {0};
      auto iss = std::istringstream(sbuf);
      for (int i = 0; i < 5; i++) {
        iss >> dis[i];
      }
    ...
    if (hfov > 0 && vfov > 0) {
      insmat(0, 2) = w * 0.5;
      insmat(1, 2) = h * 0.5;
      insmat(0, 0) = w * 0.5 / std::tan(hfov * a2r * 0.5);
      insmat(1, 1) = h * 0.5 / std::tan(vfov * a2r * 0.5);
    } else {
      std::cout << "fov invalid.\n";
    }
  } else if (sbuf == "2") {
    // if has ccd
    double wccd = 0, hccd = 0, flen = 0;
    // read ccd parameters
    GetPropValue(sensor.intrinsic().params(), "CcdWidth", wccd);
    GetPropValue(sensor.intrinsic().params(), "CCD_Width", wccd);
    GetPropValue(sensor.intrinsic().params(), "CcdHeight", hccd);
    GetPropValue(sensor.intrinsic().params(), "CCD_Height", hccd);
    GetPropValue(sensor.intrinsic().params(), "CcdFocal", flen);
    GetPropValue(sensor.intrinsic().params(), "Focal_Length", flen);

    // calculate intrinsic matrix
    if (wccd > 0 && hccd > 0) {
      insmat(0, 2) = w * 0.5;
      insmat(1, 2) = h * 0.5;
      insmat(0, 0) = w * flen / wccd;
      insmat(1, 1) = h * flen / hccd;
    } else {
      std::cout << "ccd invalid.\n";
    }
  }
