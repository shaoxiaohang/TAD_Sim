...
void APedestrianManager::Update(const FManagerIn& Input, FManagerOut& Output)
{
    ...
    for (size_t i = 0; i < TInput->pedestrianArry.Num(); i++)
    {
        ...
        if (!IsAssigned)    // Add
        {
            ...
            if (!GConfig->GetBool(TEXT("Mode"), TEXT("SnapGround"), PedestrianConfig.isEgoSnap, GGameIni))
            {
                PedestrianConfig.isEgoSnap = false;
            }
            ...
            if (UDisplayGameInstance* GI = Cast<UDisplayGameInstance>(GetWorld()->GetGameInstance()))
            {
                ...
                if (NewPedestrian)
                {
                    ...
                    UE_LOG(SimLogPedestrianManager, Error, TEXT("Add Pedestrian, ID: %d"), PedestrianConfig.id);
    ...
    for (int32 i = pedestrianArry.Num() - 1; i >= 0; i--)
    {
        if (pedestrianArry[i] && !pedestrianArry[i]->IsActorBeingDestroyed())
        {
            ...
            {
                UE_LOG(SimLogPedestrianManager, Error, TEXT("Remove Pedestrian, ID: %d TimeStamp: %f input TimeStamp: %f"),
                    pedestrianArry[i]->GetConfig()->id, pedestrianArry[i]->GetTimeStamp(), TInput->timeStamp);
