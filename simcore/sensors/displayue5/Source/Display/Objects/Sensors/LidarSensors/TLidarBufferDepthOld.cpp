...
// #define DEP_ROTATE
bool ALidarBufferDepthOld::Init(const FLidarConfig& _config, std::shared_ptr<lidar::TraditionalLidar> _LidarSensor,
...
TArray<class ADepthLidarBuffer*>& ALidarBufferDepthOld::GetDepthCameraActors()
{
    return depthCameraActors;
}

TSharedPtr<LidarBuffer> ALidarBufferDepthOld::GetTBuffer(const FTLidarMeasurement& measure)
{
    // 设置天气
    if (lidarMd)
    {
    ...
    // 获取深度数据
    TSharedPtr<DepthLidarBufferOld> buffer = MakeShared<DepthLidarBufferOld>();
...
bool ALidarBufferDepthOld::LoadStencilMap(const FString& dir)
{
    // 导入材质tag定义
    TArray<FString> txt;
...
void ALidarBufferDepthOld::getData(
...
// 获取激光点云
bool ALidarBufferDepthOld::GetPoints(
    const LidarBuffer* rawbuf, const FTLidarMeasurement& measure, lidar::TraditionalLidar::lidar_ptset& lidarBuffer)
{
    const DepthLidarBufferOld* curBuffer = StaticCast<const DepthLidarBufferOld*>(rawbuf);
...
void ALidarBufferDepthOld::setRotationTranslation(const FTransform& mat)
{
