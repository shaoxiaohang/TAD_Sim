...
const FSimActorConfig* APedestrianCharacter::GetConfig() const
{
    check(basicInfoComp);
...
void APedestrianCharacter::Init(const FSimActorConfig& _Config)
{
    ...
    //     TEXT("TPSView"));
    // }
    bool isEgoSnap = PedestrianConfig->isEgoSnap;
    ...
    {
        NewTransform = FTransform(PedestrianConfig->startRotation, PedestrianConfig->startLocation);
    }
...
void APedestrianCharacter::Update(const FSimActorInput& _Input, FSimActorOutput& _Output)
{
    ...
    {
        NewTransform = FTransform(PedestrianIn->rotation, PedestrianIn->location);
    }
