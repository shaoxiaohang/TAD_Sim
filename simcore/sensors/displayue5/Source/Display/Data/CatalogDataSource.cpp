...
bool UCatalogDataSource::LoadSceneBuffer(const std::string& Buffer)
{
...

    {
        ...
        for (int32 i = 0; i < scene.vehicles().size(); i++)
        {
            ...
            UE_LOG(SimLogCatalog, Log, TEXT("Add Vehicle Catalog typeName: %s id : %d modelPath : %s centerOffset : %s size : %s"), *TypeName, ID,
                *CommonData->ModelPath, *CommonData->CenterOffset.ToString(), *CommonData->Dimension.ToString());
        }
