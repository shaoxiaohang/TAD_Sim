...
void AMapGeneratedActor::DrawRoadDetails(TArray<LaneBoundaryInfo>& lane_bdids_in)
{
    // road lane
    ...
    for (auto& road_ptr : _roads)
    {
        for (auto section_ptr : road_ptr->getSections())
        {
            ...
            for (auto bd : bds)
            {
                ...
                auto section_ptr_tmp = bd.Value->getSection();
                auto road_ptr_tmp = section_ptr_tmp->getRoad();
                BoundaryInfo.RoadId = road_ptr_tmp->getId();
                BoundaryInfo.SectionId = section_ptr_tmp->getId();
                lane_bdids_in.Add(BoundaryInfo);
...
void AMapGeneratedActor::DrawMarkDetails(TArray<LaneBoundaryInfo>& lane_bdids_in)
{
    ...
    for (auto lbd0 : lane_bdids_in)
    {
        ...
        if (boundaryPtr)
        {
            ...
            {
                std::vector<FVector>& points_tpm = lane.points;
                if (points_tpm.empty() || normaldata.empty())
                {
                    continue;
                }
                FVector2D offp[4];
                ...
                if (points_tpm.size() > 2)
                {
                    FVector Dir = points_tpm.back() - points_tpm.front();
                    Dir.Normalize();
                    LaneDirgee = FMath::RadiansToDegrees(FMath::Atan2(Dir.Y, Dir.X));
                }
                for (size_t i = 0; i < points_tpm.size(); /*++i*/)
                {
                    if (points_tpm.size() < 2)
                        continue;
                    ...
                    if (i > 0)
                    {
                        Dis = (points_tpm[i] - lastLoc).Size() * 0.01 / 5;
    ...
    for (auto junc : juncs)
    {
        ...
        for (auto i : ids)
        {
            ...
            if (link.get() && link->getGeometry())
            {
                ...
                for (int32 left_i = 0; left_i < Left.size(); left_i++)
                {
                    Bds.push_back(Left[left_i]);
                    Bds.push_back(Right[left_i]);
                }
                ...
                for (int32 lane_idx = 0; lane_idx < int32(Bds.size() - 1); lane_idx++)
                {
                    ...
                    for (int32 l_i = 0; l_i < LeftPoints.size(); l_i++)
                    {
                        LonLatToLocal(LeftPoints[l_i].x, LeftPoints[l_i].y, LeftPoints[l_i].z);
                        LaneVertices_Left.Add(FVector(LeftPoints[l_i].x, LeftPoints[l_i].y, LeftPoints[l_i].z));
                    }
                    for (int32 r_i = 0; r_i < RightPoints.size(); r_i++)
                    {
                        LonLatToLocal(RightPoints[r_i].x, RightPoints[r_i].y, RightPoints[r_i].z);
                        LaneVertices_Right.Add(FVector(RightPoints[r_i].x, RightPoints[r_i].y, RightPoints[r_i].z));
                    }
        ...
        if (!CombinedName.IsEmpty())
        {
            ...
            if (TSubclassOf<AActor> Class = GetActor(CombinedName))
            {
                ...
                if (obj->getObjectType() == OBJECT_TYPE_PedestrianBridge)
                {
                    ...
                    //         FindField<FFloatProperty>(NewItem->GetClass(), TEXT("CustomLength"));
    ...
    bool bNewActor)
{
    ...
    bool Res = staticMesh->IsBuiltAtRuntime();
