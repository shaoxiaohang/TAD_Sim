...
void FindMeshInfo(const aiScene* scene, aiNode* node, FReturnedData& result, FTransform& tran, int32& MeshIdx)
{
    ...
    for (uint32 i = 0; i < node->mNumMeshes; i++)
    {
        ...
        FRuntimeMeshInfo& mi = result.meshInfo[MeshIdx];
        MeshIdx++;
    ...
    FReturnedData& ReturnedData, /* UProceduralMeshComponent* ProcMeshComp ,*/ FMeshDescription& MeshDescription)
{
    ...
    for (int32 SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
    {
        FRuntimeMeshInfo MeshInfo = ReturnedData.meshInfo[SectionIdx];
        // MeshInfo.Normals
...
FMeshDescription BuildMeshDescriptionExtend(FReturnedData& MeshsData /* UProceduralMeshComponent* ProcMeshComp */)
{
    ...
    for (int32 SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
    {
        FRuntimeMeshInfo MeshInfo = MeshsData.meshInfo[SectionIdx];
    ...
    for (int32 SectionIdx = 0; SectionIdx < NumSections; SectionIdx++)
    {
        ...
        for (int32 IndiceIndex = 0; IndiceIndex < NumIndices; IndiceIndex++)
        {
            ...
            Normals[VertexInstanceID] = (FVector3f)MeshInfo.Normals[VertexIndex];    // ProcVertex.Normal;
            ...
            {
                UVs.Set(VertexInstanceID, 0, (FVector2f)MeshInfo.UV0[VertexIndex]);
            }

            if (MeshInfo.UV1.Num() > VertexIndex)
            {
                UVs.Set(VertexInstanceID, 1, (FVector2f)MeshInfo.UV1[VertexIndex]);
            }

            if (MeshInfo.UV2.Num() > VertexIndex)
            {
                UVs.Set(VertexInstanceID, 2, (FVector2f)MeshInfo.UV2[VertexIndex]);
            }

            if (MeshInfo.UV3.Num() > VertexIndex)
            {
                UVs.Set(VertexInstanceID, 3, (FVector2f)MeshInfo.UV3[VertexIndex]);
            }
        }
