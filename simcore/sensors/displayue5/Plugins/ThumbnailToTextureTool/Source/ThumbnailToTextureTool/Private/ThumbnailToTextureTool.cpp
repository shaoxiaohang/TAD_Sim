				...
				for (const FAssetData& AssetData : SelectedAssets)
				{
					...
					if (MyObject)
					{
						...
						if (ObjectThumbnail)
						{
							...
							if (ImageWrapper)
							{
								UE_LOG(LogTemp, Log, TEXT("ImageWrapper OK"));
								const TArray64<uint8>& CompressedByteArray = ImageWrapper->GetCompressed();
								FFileHelper::SaveArrayToFile(CompressedByteArray, *PackageName);
							}
						}
					}
