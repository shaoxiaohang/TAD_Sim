...
{
    shared_segment_ = MakeShared<SharedSegment>();
}
...
bool SharedMemoryWriter::reset()
{
    try
    {
        ...
        // shared_segment_->mtx_ = shared_segment_->segment_.
        // find_or_construct<boost::interprocess::interprocess_sharable_mutex> \
        // (mtx_key_.c_str())();
        if (shared_segment_->ptr_)
            printf("SharedMemory write init success\n");
        return /*shared_segment_->mtx_ && */ shared_segment_->ptr_ != nullptr;
    }
    catch (...)
    {
