...
std::shared_ptr<TxCar_EV> gCarModel;
ExtU_TxCar_EV_T gCarU;
ExtY_TxCar_EV_T gCarY;

///////////////////////
TxCarImp_EV::TxCarImp_EV() {}
TxCarImp_EV::~TxCarImp_EV() {}

void TxCarImp_EV::setCarInit(const TxCarInit& car_init) { m_carParam = car_init; }

void TxCarImp_EV::resetTerrain() {
...
void TxCarImp_EV::resetVehicle() {
  ...
  // init
  gCarModel = std::make_shared<TxCar_EV>();
  memset(&gCarU, 0x00, sizeof(ExtU_TxCar_EV_T));
  gCarModel->initialize();
}

void TxCarImp_EV::initMDL() {
  // init values
  {
    mMsgs.clear();
    mCgToFrontAxle = 1.6;
...
bool TxCarImp_EV::isModelValid() { return (gCarModel.get() != nullptr); }

void TxCarImp_EV::init(const TxCarInit& car_init) {
  setCarInit(car_init);
  initMDL();
  m_ctrl_watch_dog = MsgWatchDog();
  m_ctrlV2_watch_dog = MsgWatchDog();
}
void TxCarImp_EV::step(double t_ms) {
...
void TxCarImp_EV::stop() {
  gCarModel->terminate();
  mTerrain.releaseRoad();
}

void TxCarImp_EV::controlToVehicle(ExtU_TxCar_EV_T& car_u, const std::string& ctrl_payload) {
...
void TxCarImp_EV::vehicleToChassis(double t_ms, const ExtU_TxCar_EV_T& car_u, const ExtY_TxCar_EV_T& vehOut,
  ...
  // powertrain state
  {
    ...
    } else if (car_u.DriverIn.gear_cmd__0N1D2R3P == 0) {
    ...
    } else if (static_cast<int64_t>(car_u.DriverIn.gear_cmd__0N1D2R3P) == 2) {
  ...
  // chassis state
  {
    ...
    // set wheel contact
    {
      ...
      auto SetWheelContact = [this](sim_msg::Vec3* wheel_contact, const ExtY_TxCar_EV_T& vehOut, Eigen::Vector3d& pos) {
...
void TxCarImp_EV::vehicleToLocation(double t_ms, const ExtY_TxCar_EV_T& vehOut, std::string& loc_payload) {
...
Eigen::Vector3d TxCarImp_EV::getOffsetFromCG() {
...
Eigen::Vector3d TxCarImp_EV::calWheelsZ(const Eigen::Vector3d& FLU_Pos, const Eigen::Matrix3d& FLU_Rot, double l_offset,
                                        double w_offset, double& m_mu) {
...
Declare_Vehicle_Model(tx_car::TxCarImp_EV)
